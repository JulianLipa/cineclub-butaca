...
export async function getMoviePreview(id) {
  const API = getApiKey();
  const res = await fetch(`${BASE}/movie/${id}?api_key=${API}&language=es-AR`);
...
export async function getMovieById(id) {
  ...
  const [detailsRes, creditsRes, videosRes] = await Promise.all([
    fetch(`${BASE}/movie/${id}?api_key=${API}&language=es-AR`),
    fetch(`${BASE}/movie/${id}/credits?api_key=${API}&language=es-AR`),
    fetch(`${BASE}/movie/${id}/videos?api_key=${API}&language=es-AR,en-US`),
  ]);
