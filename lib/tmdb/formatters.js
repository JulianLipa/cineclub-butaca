...
export function formatMovie(details, credits, videos, providers) {
  const director = credits.crew.find((c) => c.job === "Director");
  const trailer = findTrailer(videos);
  const arProviders = providers?.results?.AR;
  const streaming = [
    ...(arProviders?.flatrate ?? []),
    ...(arProviders?.free ?? []),
  ]
    ...
    .map(({ provider_id, provider_name, logo_path }) => ({
      ...
      logo: logo_path ? `${IMG}/w92${logo_path}` : null,
    }));
  ...
  return {
    ...
    streaming: streaming.length > 0 ? streaming : null,
  };
}
